...
let flashed = {}
const STEPS = 1000
const result = flashSquid(input, STEPS)

console.log(`Result: ${isNaN(result) ? result : STEPS - result}`)

function flashSquid(input, steps) {
	const ALL = input.length * input[0].length
	...
	while(--steps >= 0) {
		...
		if(Object.keys(flashed).length === ALL) {
			return steps
		}
	}
