...
try {
	...
	let boards = parseData(data)

	while(boards.length > 0) {
		const [boardIndex, result] = playBingo(draw, boards)

		boards = boards.filter((b, idx) => idx !== boardIndex)

		console.log(`${boardIndex} = ${result} (remaining) = ${boards.length}`)
	}
	...
	for(let i = 0, ilen = draw.length; i < ilen; ++i) {
		for(let j = 0, jlen = boards.length; j < jlen; ++j) {
			...
			if(checkRowAndCol(boards[j])) {
				return [j, getWinningNumbers(boards[j]) * draw[i]]
			}
		}
	}
